...
class TestTopicArticleMatchingBug:
    ...
    def setup_test_data(self, db_connection):
        ...
        with db_connection.get_connection() as conn:
            ...
            """,
                (
                    chat_id,
                    "AWS ECS Technical Updates",
                    '["aws ecs", "container orchestration", "microservices"]',
                ),
            )
            ...
            """,
                (
                    ...
                    '["leadership", "engineering management", "team culture"]',
                ),
            )
    ...
    @pytest.mark.asyncio
    async def test_bug_reproduction_all_articles_to_all_topics(
        self, message_sender, db_connection
    ):
        """
        ...
        articles_by_topic = await message_sender._get_articles_for_delivery(
            chat_id, limit_per_topic=10
        )
        ...
        assert (
            len(articles_by_topic) == 0
        ), "Fixed: Empty processing_results correctly returns no articles to prevent bug"
    ...
    async def test_expected_behavior_after_fix(self, message_sender, db_connection):
        ...
        leadership_topic_articles = articles_by_topic.get(
            "Engineering Culture & Leadership", []
        )

        # FIXED BEHAVIOR: Each topic gets only its relevant articles
        assert (
            len(aws_topic_articles) == 1
        ), "AWS topic should get exactly 1 AWS article"
        assert (
            len(leadership_topic_articles) == 1
        ), "Leadership topic should get exactly 1 leadership article"
        ...
        assert (
            "psychological safety" in leadership_article.title
        ), "Leadership topic should get leadership article"

        # The fix: topics have different articles
        assert (
            aws_article.id != leadership_article.id
        ), "Topics should have different articles"
    ...
    @pytest.mark.asyncio
    async def test_empty_processing_results_causes_bug(
        self, message_sender, db_connection
    ):
        ...
        if len(articles_by_topic) > 1:
            ...
            ), "Bug confirmed: empty processing_results causes all topics to get same articles"

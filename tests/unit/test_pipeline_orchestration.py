...
class TestPipelineOrchestration:
    ...
    def sample_feed_data(self, db_connection):
        ...
        topics = [
            ...
            Topic(
                ...
                keywords=["javascript", "react", "frontend", "web development", "nodejs"],
    ...
    @pytest.mark.asyncio
    async def test_full_pipeline_integration_success(self, real_pipeline, sample_feed_data, sample_articles):
        ...
        fetch_results = [
            ...
            FetchResult(
                ...
                success=True,
                articles=[sample_articles[3], sample_articles[4]],  # Duplicate and irrelevant
                error=None,
                fetch_time=1.2
            )
        ]
        
        with patch.object(real_pipeline.feed_fetcher, 'fetch_feeds_batch', new_callable=AsyncMock) as mock_fetch:
    ...
    @pytest.mark.asyncio
    async def test_pipeline_with_feed_failures(self, real_pipeline, sample_feed_data, sample_articles):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_data_flow_integrity(self, real_pipeline, sample_feed_data, sample_articles):
        ...
        with real_pipeline.db.get_connection() as conn:
            stored_count = conn.execute("SELECT COUNT(*) as count FROM articles").fetchone()['count']
            ...
            assert ai_article['title'] == "Machine Learning and Artificial Intelligence Tutorial"

    @pytest.mark.asyncio
    async def test_pipeline_component_coordination(self, real_pipeline, sample_feed_data):
    ...
    async def test_pipeline_error_propagation(self, real_pipeline, sample_feed_data):
        ...
        with patch.object(real_pipeline.article_processor, 'process_articles') as mock_processor:
    ...
    @pytest.mark.asyncio
    async def test_pipeline_performance_measurement(self, real_pipeline, sample_feed_data, sample_articles):
        ...
        with patch.object(real_pipeline.feed_fetcher, 'fetch_feeds_batch', side_effect=slow_fetch):
        ...
        assert result.processing_time_seconds <= total_elapsed + 0.1  # Allow small tolerance
    ...
    @pytest.mark.asyncio
    async def test_multi_channel_orchestration(self, real_pipeline, sample_feed_data, sample_articles):
    ...
    async def test_pipeline_resource_management(self, real_pipeline, sample_feed_data):
        ...
        with real_pipeline.db.get_connection() as conn:
            initial_article_count = conn.execute("SELECT COUNT(*) as count FROM articles").fetchone()['count']
        ...
        with real_pipeline.db.get_connection() as conn:
            final_article_count = conn.execute("SELECT COUNT(*) as count FROM articles").fetchone()['count']
            ...
            if result.articles_ready_for_ai > 0:
                ...
                if stored_article:
                    assert stored_article['title'] == "Test Article for Resource Management"
    ...
    async def test_pipeline_edge_cases(self, real_pipeline, sample_feed_data):
        ...
        with real_pipeline.db.get_connection() as conn:
            conn.execute("UPDATE feeds SET active = 0 WHERE chat_id = ?", ("test_channel",))
            conn.commit()
        ...
        with real_pipeline.db.get_connection() as conn:
            conn.execute("UPDATE feeds SET active = 1 WHERE chat_id = ?", ("test_channel",))
            conn.commit()
        
        # Test 3: Channel with no active topics
        with real_pipeline.db.get_connection() as conn:
            conn.execute("UPDATE topics SET active = 0 WHERE chat_id = ?", ("test_channel",))
            conn.commit()
    ...
    @pytest.mark.asyncio
    async def test_pipeline_concurrent_safety(self, real_pipeline, sample_feed_data, sample_articles):
    ...
    @pytest.mark.asyncio
    async def test_daily_processing_orchestration(self, real_pipeline, sample_feed_data):
        ...
        with patch.object(real_pipeline, 'process_multiple_channels', new_callable=AsyncMock) as mock_multi:

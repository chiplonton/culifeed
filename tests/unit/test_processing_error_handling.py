...
from culifeed.utils.exceptions import (
    FeedFetchError, ProcessingError, DatabaseError, 
...
class TestProcessingErrorHandling:
    ...
    def test_feed_fetcher_invalid_content_errors(self):
        ...
        for invalid_content in invalid_feeds:
            with patch('requests.get') as mock_get:
                ...
                error = exc_info.value
                assert error.recoverable is True  # Feed errors are recoverable by default
    ...
    def test_article_processor_memory_limits(self, db_connection):
        ...
        try:
            unique_articles, stats = processor.process_articles(large_articles, check_database=False)
    ...
    async def test_pipeline_component_failures(self, pipeline):
        ...
        with patch.object(pipeline.feed_fetcher, 'fetch_feeds_batch', new_callable=AsyncMock) as mock_fetch:
    ...
    @pytest.mark.asyncio 
    async def test_pipeline_database_transaction_failures(self, pipeline, sample_articles):
        ...
        with pipeline.db.get_connection() as conn:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (1, chat_id, "Test Topic", '["test"]', '[]', True, datetime.now(timezone.utc)))
            conn.commit()
    ...
    async def test_pipeline_timeout_handling(self, pipeline, sample_articles):
        ...
        with patch.object(pipeline.feed_fetcher, 'fetch_feeds_batch', side_effect=slow_fetch):
            ...
            try:
                result = await asyncio.wait_for(timeout_task, timeout=1.0)  # 1 second timeout
    ...
    def test_article_processor_invalid_content_hash(self, db_connection):
        ...
        unique_articles, stats = processor.process_articles(problematic_articles, check_database=False)
    ...
    def test_pre_filter_engine_performance_degradation(self):
        ...
        assert processing_time < 5.0  # Should complete within reasonable time  # Should complete within reasonable time
    ...
    async def test_pipeline_concurrent_error_isolation(self, pipeline, sample_articles):
        ...
        with pipeline.db.get_connection() as conn:
            ...
            for chat_id in chat_ids:
                conn.execute("""
                    INSERT OR IGNORE INTO channels (chat_id, chat_title, chat_type, created_at)
                    VALUES (?, ?, ?, ?)
                """, (chat_id, f"Test Channel {chat_id}", "group", datetime.now(timezone.utc)))
            ...
            for chat_id in chat_ids:
                conn.execute("""
                    INSERT INTO feeds (chat_id, url, title, active, created_at)
                    VALUES (?, ?, ?, ?, ?)
                """, (chat_id, f"https://example.com/{chat_id}/feed.xml", f"Feed for {chat_id}", True, datetime.now(timezone.utc)))
            conn.commit()
        ...
        with patch.object(pipeline, 'process_channel', side_effect=mock_process_channel):
    ...
    def test_configuration_error_handling(self, db_connection):
        ...
        with pytest.raises((FileNotFoundError, OSError)):  # Fix: Expect FileNotFoundError instead of DatabaseError
    ...
    async def test_resource_exhaustion_handling(self, pipeline):
        ...
        with patch.object(pipeline.feed_fetcher, 'max_concurrent', 1):  # Limit concurrency
            with patch.object(pipeline.feed_fetcher, 'fetch_feeds_batch', new_callable=AsyncMock) as mock_fetch:
    ...
    async def test_graceful_degradation_scenarios(self, pipeline, sample_articles):
        ...
        with patch.object(pipeline.feed_fetcher, 'fetch_feeds_batch', new_callable=AsyncMock) as mock_fetch:
            ...
            assert result.processing_time_seconds > 0  # Should still measure time  # Should still measure time
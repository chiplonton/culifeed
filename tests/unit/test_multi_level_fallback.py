...
def test_configuration_multi_models():
    ...
    try:
        ...
        assert hasattr(
            settings.ai, "gemini_models"
        ), "Missing gemini_models configuration"
        assert hasattr(
            settings.ai, "deepseek_models"
        ), "Missing deepseek_models configuration"
        assert hasattr(
            settings.ai, "get_models_for_provider"
        ), "Missing get_models_for_provider method"
        ...
        assert (
            "llama-3.1-8b-instant" in groq_models
        ), "Should include default Groq model"
        ...
        deepseek_models = settings.ai.get_models_for_provider(ConfigAIProvider.DEEPSEEK)
        assert isinstance(deepseek_models, list), "DeepSeek models should be a list"
        assert len(deepseek_models) >= 1, "Should have at least one DeepSeek model"
        assert (
            "deepseek-chat" in deepseek_models
        ), "Should include confirmed working model"
        ...
        with patch.object(AIManager, "_initialize_providers"):
            ...
            with patch.object(
                ...
                for combo in combinations:
                    assert isinstance(
                        combo, tuple
                    ), f"Each combination should be a tuple: {combo}"
                    assert (
                        len(combo) == 2
                    ), f"Each combination should have 2 elements: {combo}"
                    provider_type, model_name = combo
                    assert isinstance(
                        provider_type, str
                    ), f"Provider type should be string: {provider_type}"
                    assert isinstance(
                        model_name, str
                    ), f"Model name should be string: {model_name}"
...
def test_groq_provider_multi_model():
    ...
    try:
        ...
        with patch("culifeed.ai.providers.groq_provider.GROQ_AVAILABLE", True):
            with patch("culifeed.ai.providers.groq_provider.Groq"), patch(
                "culifeed.ai.providers.groq_provider.AsyncGroq"
            ):
                ...
                provider = GroqProvider(
                    api_key="test-key", model_name="llama-3.1-8b-instant"
                )
                ...
                assert (
                    provider.model_name == "llama-3.1-70b-versatile"
                ), "Model should have switched"

                # Test invalid model (should keep current)
                provider.set_model("invalid-model")
                assert (
                    provider.model_name == "llama-3.1-70b-versatile"
                ), "Should keep current model for invalid"
...
def test_gemini_provider_basic_structure():
    ...
    try:
        ...
        for method_name in required_methods:
            ...
            ), f"Should have {method_name} method"
...
def test_deepseek_provider_basic_structure():
    """Test DeepSeekProvider basic structure and available models."""
    print("🧠 Testing DeepSeekProvider basic structure...")

    try:
        from culifeed.ai.providers.deepseek_provider import DeepSeekProvider

        # Test get_available_models (static method)
        models = DeepSeekProvider.get_available_models()
        assert isinstance(models, list), "Available models should be a list"
        assert len(models) > 0, "Should have available models"
        assert "deepseek-chat" in models, "Should include confirmed working model"
        ...
        for method_name in required_methods:
            assert hasattr(
                DeepSeekProvider, method_name
            ), f"Should have {method_name} method"

        print(f"  ✅ Available DeepSeek models: {models}")
        print(f"  ✅ All required methods present")
        return True

    except Exception as e:
        print(f"  ❌ DeepSeekProvider basic structure test failed: {e}")
        return False
...
async def test_ai_manager_fallback_logic():
    ...
    try:
        ...
        with patch.object(AIManager, "_initialize_providers"):
            ...
            mock_groq.analyze_relevance_with_model = AsyncMock(
            ...
            with patch.object(
                ai_manager,
                "_get_provider_model_combinations",
                return_value=[
                ...
                ), "Should return correct relevance score"
...
def test_error_handling():
    ...
    try:
        ...
        assert (
            error.provider == "groq/llama-3.1-70b-versatile"
        ), "Should store provider info"
        assert (
            error.error_code == ErrorCode.AI_PROCESSING_ERROR
...
def run_all_tests():
    ...
    tests = [
        ...
        ("DeepSeekProvider Basic Structure", test_deepseek_provider_basic_structure),

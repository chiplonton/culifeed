...
def create_test_article(
    title: str, content: str = "", url: str = "https://example.com"
) -> Article:
    """Helper to create test Article objects."""
    return Article(
        title=title, content=content, url=url, source_feed="https://test-feed.com"
    )
...
def create_test_topic(name: str, keywords: list, chat_id: str = "test_chat") -> Topic:
    ...
    return Topic(
        chat_id=chat_id, name=name, keywords=keywords, confidence_threshold=0.7
    )
...
class TestSmartRoutingRealWorld:
    ...
    def test_problematic_article_should_be_rejected(self):
        ...
        specific_matches = [
            kw for kw in result.matched_keywords if kw.lower() in tiktok_specific
        ]
        assert (
    ...
    def test_aws_ecs_article_specificity(self):
        ...
        specific_result = analyzer.analyze_article_confidence(
            specific_article, aws_topic
        )
        generic_result = analyzer.analyze_article_confidence(generic_article, aws_topic)

        print(f"\nAWS Specificity Test:")
        print(
            f"Specific ECS Article - Confidence: {specific_result.confidence_level:.3f}, Score: {specific_result.relevance_score:.3f}"
        )
        print(
            f"Generic AWS Article - Confidence: {generic_result.confidence_level:.3f}, Score: {generic_result.relevance_score:.3f}"
        )

        # Specific ECS article should have higher confidence than generic AWS
        assert (
            specific_result.confidence_level >= generic_result.confidence_level
        ...
        ecs_matches = [
            kw
            for kw in specific_result.matched_keywords
            if kw.lower() in container_keywords
        ]
        assert (
            len(ecs_matches) >= 1

...
def create_test_article(
    title: str, content: str = "", summary: str = "", url: str = "https://example.com"
) -> Article:
...
class TestSmartKeywordAnalyzer:
    ...
    def test_tiktok_specific_articles_match_correctly(self, analyzer, test_topics):
        ...
        for article in tiktok_articles:
            ...
            ), f"TikTok article should have high confidence: {article.title} (got {result.confidence_level})"
            assert (
                result.relevance_score >= 0.6
            ), f"TikTok article should have high score: {article.title} (got {result.relevance_score})"
            assert (
            ...
            assert any(
                kw.lower() in tiktok_specific for kw in result.matched_keywords
    ...
    def test_generic_programming_articles_rejected(self, analyzer, test_topics):
        ...
        for article in generic_articles:
            ...
            assert (
                result.confidence_level < 0.7
            ), f"Generic article should have low confidence: {article.title} (got {result.confidence_level})"
            ...
            if result.matched_keywords:
                ...
                specific_matches = [
                    ...
                    if kw.lower() in tiktok_specific
                ]
                assert (
    ...
    def test_aws_ecs_eks_articles_match_correctly(self, analyzer, test_topics):
        ...
        for article in aws_articles:
            ...
            assert (
                result.relevance_score >= 0.6
            ), f"AWS article should have high score: {article.title}"
            ...
            assert any(
                kw.lower() in aws_specific for kw in result.matched_keywords

...
class TestFeedManager:
        ...
        async def mock_fetch_with_error_handling(feed_url, session):
            if "example1" in feed_url:
                ...
                feed_metadata = FeedMetadata(
                    title=f"Feed {feed_url}", link=feed_url, description="Test feed"
                )
                articles = [
                    ParsedArticle(
    ...
    def test_content_length_validation(self):
        ...
        with patch.object(
            self.feed_manager.settings.processing, "max_content_length", 1000
        ):
...
class TestFeedMetadata:
    ...
    def test_metadata_creation(self):
        ...
        metadata = FeedMetadata(
            title="Test Feed", link="http://example.com", description="Test description"
        )
...
class TestIntegration:
    ...
    def test_feed_with_html_content(self, mock_get):
        ...
        # Fetch feed
        feed_metadata, articles = self.feed_manager.fetch_feed(

...
class TestPreFilterEngine:
    ...
    def test_extract_text_features_empty_content(self, engine, sample_articles):
        ...
        assert (
            features["clean_text"] == "empty article"
        )  # Title is used when content is empty
    ...
    def test_calculate_keyword_relevance_partial_match(self, engine):
        ...
        text_features = {
            ...
            "word_counts": Counter(
                ["modern", "javascript", "frameworks", "web", "development"]
            ),
            "tf_scores": {
                "modern": 0.2,
                "javascript": 0.2,
    ...
    def test_check_exclusion_keywords_match(self, engine):
        ...
        text_features = {
            ...
            "word_counts": Counter(
                ["cryptocurrency", "trading", "bitcoin", "mining", "guide"]
            ),
            "tf_scores": {},
        }
        ...
        excluded = engine._check_exclusion_keywords(
            text_features, ["spam", "advertisement"]
        )
    ...
    def test_filter_article_excluded(self, engine, sample_articles, sample_topics):
        ...
        crypto_article = sample_articles[
            2
        ]  # Crypto article (has AI but should be excluded)
    ...
    def test_filter_article_below_threshold(
        self, engine, sample_articles, sample_topics
    ):
    ...
    def test_filter_article_inactive_topics(
        self, engine, sample_articles, sample_topics
    ):
    ...
    def test_filter_articles_performance_logging(
        self, engine, sample_articles, sample_topics, caplog
    ):
        ...
        assert any(
            "Pre-filtering" in msg and "articles against" in msg for msg in log_messages
        )

        # Should log filtering completion with reduction percentage
        assert any(
            "Pre-filtering complete" in msg and "reduction" in msg
    ...
    def test_get_filtered_articles(self, engine, sample_articles, sample_topics):
        ...
        assert set(a.id for a in filtered_articles) == set(
            a.id for a in passed_articles
        )
    ...
    def test_filter_result_properties(self, engine, sample_articles, sample_topics):
        ...
        if result.relevance_scores:
            expected_best_topic = max(
                result.relevance_scores.keys(), key=lambda k: result.relevance_scores[k]
            )

...
class TestAIKeywordGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_keywords_with_ai_success(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
        ...
        keywords = await topic_handler._generate_keywords_with_ai(
            "Machine Learning", "test_chat_123"
        )
        ...
        mock_ai_manager.generate_keywords.assert_called_once_with(
            "Machine Learning", "", max_keywords=7
        )

    @pytest.mark.asyncio
    async def test_generate_keywords_with_context(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
        ...
        keywords = await topic_handler._generate_keywords_with_ai(
            "React Development", "test_chat_123"
        )
        ...
        mock_ai_manager.generate_keywords.assert_called_once_with(
            "React Development", "", max_keywords=7
        )

    @pytest.mark.asyncio
    async def test_generate_keywords_ai_failure_fallback(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_keywords_exception_fallback(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_keywords_empty_content_fallback(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
        ...
        assert (
            len(keywords) == 2
        )  # Falls back to [topic.lower(), f"{topic.lower()} technology"]
        assert keywords[0] == "blockchain"
        assert keywords[1] == "blockchain technology"

    @pytest.mark.asyncio
    async def test_generate_keywords_limits_to_max(
        self, topic_handler, mock_ai_manager, mock_topic_repo
    ):
    ...
    def test_parse_add_topic_args_manual_keywords(self, topic_handler):
        ...
        result = topic_handler._parse_add_topic_args(
            ["Cloud,", "AWS,", "Azure,", "GCP"]
        )
        assert result is not None
        topic_name, keywords = result

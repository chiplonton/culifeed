...
def test_feedparser_integration():
    ...
    try:
        ...
        ), f"Expected 'Test Feed', got '{parsed.feed.title}'"
        assert len(parsed.entries) == 1, f"Expected 1 entry, got {len(parsed.entries)}"
        assert (
            parsed.entries[0].title == "Test Article"
        ), f"Expected 'Test Article', got '{parsed.entries[0].title}'"
...
def test_feed_manager_basic():
    ...
    try:
        ...
        assert hasattr(
            feed_manager, "_last_fetch_times"
        ), "Should have _last_fetch_times dict"

        # Test error tracking
        test_url = "http://example.com/feed"

        # Initially no errors
        assert not feed_manager._should_skip_feed(
            test_url
        ), "Should not skip feed initially"
        ...
        assert (
            test_url not in feed_manager._feed_errors

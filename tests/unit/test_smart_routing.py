...
class TestSmartKeywordAnalyzer:
    ...
    def test_tiktok_specific_articles_match_correctly(self, analyzer, test_topics):
        ...
        for article in tiktok_articles:
            ...
            confidence, score, matched_keywords = result.confidence_level, result.relevance_score, result.matched_keywords
            
            # Should have high confidence and score for TikTok-specific content
            assert confidence >= 0.7, f"TikTok article should have high confidence: {article.title}"
            assert score >= 0.6, f"TikTok article should have high score: {article.title}"
            assert len(matched_keywords) >= 2, f"TikTok article should match multiple keywords: {article.title}"
            # Must include TikTok-specific keywords
            tiktok_specific = ["tiktok", "social media", "app development"]
            assert any(kw.lower() in tiktok_specific for kw in matched_keywords), \
    ...
    def test_generic_programming_articles_rejected(self, analyzer, test_topics):
        ...
        for article in generic_articles:
            ...
            assert confidence < 0.7, f"Generic article should have low confidence: {article.title} (got {confidence})"
            
            # If it matches keywords, they should be generic ones only
            if matched_keywords:
                tiktok_specific = ["tiktok", "social media", "app development"]
                specific_matches = [kw for kw in matched_keywords if kw.lower() in tiktok_specific]
    ...
    def test_aws_ecs_eks_articles_match_correctly(self, analyzer, test_topics):
        ...
        for article in aws_articles:
            ...
            assert confidence >= 0.7, f"AWS article should have high confidence: {article.title}"
            ...
            assert any(kw.lower() in aws_specific for kw in matched_keywords), \
    ...
    def test_generic_aws_articles_get_penalized(self, analyzer, test_topics):
        ...
        for article in generic_aws_articles:
            ...
            assert confidence < 0.8, f"Generic AWS article should have reduced confidence: {article.title}"
            
            # Should match generic AWS keywords but not specific ones
            aws_specific = ["aws ecs", "aws eks", "amazon ecs", "amazon eks", "kubernetes", "container", "containerization"]
            specific_matches = [kw for kw in matched_keywords if kw.lower() in aws_specific]
    ...
    def test_single_ambiguous_keyword_penalty(self, analyzer, test_topics):
        ...
        ambiguous_test_cases = [
            (test_topics[0], "TikTok's software engineers", 
             MockArticle("Generic coding tutorial", "This is about coding in general")),
            (test_topics[1], "AWS topic",
             MockArticle("Amazon marketplace selling tips", "Amazon marketplace and selling on amazon")),
            (test_topics[2], "Engineering culture",
             MockArticle("Software development lifecycle", "About professional development methodologies"))
        ]
    ...
    def test_multiple_related_keywords_boost_confidence(self, analyzer, test_topics):
        ...
        result = analyzer.analyze_article_confidence(multi_keyword_article, tiktok_topic)
        confidence, score, matched_keywords = result.confidence_level, result.relevance_score, result.matched_keywords
        ...
        tiktok_specific = ["tiktok", "social media", "app development"]
        specific_matches = [kw for kw in matched_keywords if kw.lower() in tiktok_specific]
        assert len(specific_matches) >= 2, "Should match multiple TikTok-specific keywords"
    ...
    def test_topic_coherence_check(self, analyzer, test_topics):
        ...
        incoherent_cases = [
            (test_topics[0], "Programming tutorial for beginners", 
             "Learn programming and software engineering basics with coding examples"),
            (test_topics[1], "AWS account billing and cost optimization",
             "Amazon aws billing optimization and cloud computing cost management"),
            (test_topics[2], "Professional development in marketing",
    ...
    def test_confidence_thresholds_and_routing_decisions(self, analyzer, test_topics):
        ...
        test_cases = [
            ...
            (test_topics[0], 
             MockArticle("TikTok's new social media algorithm for app development",
        ...
        for topic, article, expected_min_confidence, confidence_level in test_cases:
            ...
            if confidence_level == "high":
                assert confidence >= 0.7, f"High confidence case failed: {article.title}"
            elif confidence_level == "medium":
                assert 0.6 <= confidence < 0.8, f"Medium confidence case failed: {article.title}"
            else:  # low
    ...
    def test_url_quality_scoring(self, analyzer, test_topics):
        ...
        if high_quality_scores and low_quality_scores:
            ...
            assert avg_high >= avg_low - 0.1, "High quality URLs should not score significantly lower"
...
class TestSmartRoutingIntegration:
    ...
    def mock_pipeline_components(self):
        ...
        with patch('culifeed.processing.smart_analyzer.get_settings') as mock_settings:
            ...
            mock_settings.return_value.smart_processing.definitely_relevant_threshold = 0.7
            mock_settings.return_value.smart_processing.definitely_irrelevant_threshold = 0.3
    ...
    def test_smart_routing_decision_logic(self, mock_pipeline_components):
        ...
        for confidence, score, expected_decision in decision_test_cases:
            ...
            with patch.object(analyzer, 'analyze_article_confidence', return_value=(confidence, score, ["test"])):

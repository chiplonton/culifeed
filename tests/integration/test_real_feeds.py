...
from culifeed.ingestion.feed_manager import FeedManager, fetch_single_feed, fetch_feeds_batch
...
def test_individual_real_feed(feed_url, feed_manager, logger):
    ...
    assert len(articles) > 0, f"Feed should have at least one article, got {len(articles)}"
    ...
    # Performance check
    assert processing_time < 30.0, f"Processing should complete within 30s, took {processing_time:.2f}s"
    
    logger.info(f"✅ Successfully processed {len(articles)} articles from {feed_metadata.title}")
...
def test_concurrent_feed_fetching(logger):
    ...
    async def run_concurrent_test():
        ...
        for feed_url, (feed_metadata, articles) in results.items():
            assert feed_metadata is not None, f"Feed metadata should not be None for {feed_url}"
            assert isinstance(articles, list), f"Articles should be a list for {feed_url}"
    ...
    assert concurrent_time < 60.0, f"Concurrent fetch should complete within 60s, took {concurrent_time:.2f}s"
    logger.info(f"✅ Successfully fetched {len(results)} feeds concurrently")

@pytest.mark.parametrize("problematic_url", PROBLEMATIC_FEEDS[:2])  # Test first 2 problematic feeds
...
def test_content_cleaning_with_real_data(content_cleaner, logger):
    ...
    try:
        ...
        if article.summary and '<' in article.summary and '>' in article.summary:
            ...
            assert cleaned_length <= original_length, "Cleaned text should not be longer than original"
            ...
            logger.info(f"Summary cleaning: {original_length} -> {cleaned_length} chars")
...
def test_feed_format_detection(logger):
    ...
    assert total_detected > 0, "Should have successfully detected at least one feed format"
...
def test_comprehensive_feed_statistics(logger):
    ...
    for feed_url in test_feeds:
        ...
        try:
            ...
            logger.info(f"  ✅ Success: {feed_metadata.title} ({len(articles)} articles)")
    ...
    assert success_rate >= 0.5, f"Success rate ({success_rate*100:.1f}%) should be at least 50%"
    assert results['total_articles'] > 0, "Should have processed at least some articles"
    assert avg_processing_time < 15.0, f"Average processing time ({avg_processing_time:.2f}s) should be reasonable"
    ...
    logger.info(f"  Articles with HTML: {results['content_stats']['articles_with_html']}")
    logger.info(f"  Articles with links: {results['content_stats']['articles_with_links']}")

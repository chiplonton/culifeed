...
def test_workflow_feed(feed_url):
    ...
    assert (
        article_count > 0
    ), f"Feed should have at least one article, got {article_count}"
    assert (
        processing_time < 30.0
    ), f"Processing time should be under 30s, got {processing_time:.2f}s"
...
def test_workflow_feeds_statistical_analysis():
    ...
    avg_processing_time = (
        total_processing_time / len(successful_feeds) if successful_feeds else 0
    )
    ...
    assert (
        success_rate >= 0.75
    ), f"Success rate ({success_rate*100:.1f}%) should be at least 75%"
    assert (
        avg_processing_time < 10.0
    ), f"Average processing time ({avg_processing_time:.2f}s) should be under 10s"
...
def test_optional_feeds(feed_url):
    ...
    try:
        ...
        assert (
            article_count >= 0
        ), f"Feed should have valid article count, got {article_count}"

...
async def test_complete_integration():
    ...
    try:
        ...
        assert settings.ai.groq_models == [
            "llama-3.1-70b-versatile",
            "llama-3.1-8b-instant",
        ]
        assert settings.ai.gemini_models == [
            "gemini-2.5-flash",
    ...
    try:
        ...
        with patch.object(AIManager, "_initialize_providers"):
            ...
            with patch.object(
                ...
                print(
                    f"   ✅ Generated correct combinations: {len(combinations)} total"
                )
    ...
    try:
        ...
        with patch.object(AIManager, "_initialize_providers"):
            ...
            ai_manager.provider_health = {
                "groq": Mock(
                    is_healthy=True, record_error=Mock(), record_success=Mock()
                ),
                "gemini": Mock(is_healthy=True),
            }
            ...
            with patch.object(
                ...
                print(
                    f"      📊 Result: score={result.relevance_score}, confidence={result.confidence}"
                )
    ...
    try:
        ...
        assert hasattr(
            settings.ai, "groq_model"
        ), "Legacy groq_model field should exist"
        assert hasattr(
            settings.ai, "gemini_model"
        ), "Legacy gemini_model field should exist"

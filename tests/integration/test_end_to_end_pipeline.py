...
async def test_end_to_end_with_mock_feeds():
    ...
    try:
        ...
        print(
            f"  {'✅' if result.articles_ready_for_ai >= 0 else '❌'} AI processing integration"
        )
...
async def test_ai_fallback_functionality():
    ...
    try:
        ...
        if summary_result.success:
            print(
                f"    ✅ Fallback summarization successful with {summary_result.provider}"
            )
            print(f"    ✅ Summary: {summary_result.summary[:100]}...")
        else:
            print(
                f"    ❌ Fallback summarization failed: {summary_result.error_message}"
            )
...
async def test_real_api_integration():
    ...
    if not api_key or api_key.startswith("test"):
        ...
        print(
            "    Set CULIFEED_AI__GROQ_API_KEY environment variable for real API testing"
        )
        return True

...
class RetryManager:
            ...
            try:
                ...
                if self.error_handler and context:
                    ...
                    self.error_handler.handle_error(
                        e, error_context, attempt_recovery=False
                    )
    ...
    def _calculate_delay(self, attempt: int, config: RetryConfig) -> float:
        ...
        calculator = self._delay_calculators.get(
            config.strategy, self._calculate_exponential_delay
        )
        ...
        if config.jitter and config.strategy not in (
    ...
    def _calculate_jittered_exponential_delay(
        self, attempt: int, config: RetryConfig
    ) -> float:
        """Exponential backoff with full jitter."""
        exponential_delay = config.base_delay * (
...
def get_retry_manager(
    config: Optional[RetryConfig] = None, error_handler: Optional[ErrorHandler] = None
) -> RetryManager:

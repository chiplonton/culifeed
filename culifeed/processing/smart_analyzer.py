...
class SmartKeywordAnalyzer:
    ...
    def analyze_article_confidence(
        ...
        reasoning.append(
            f"Keyword matching: {keyword_score:.2f} (matched: {', '.join(matched_keywords) if matched_keywords else 'none'})"
        )
        ...
        routing_decision = self._make_routing_decision(
            relevance_score, confidence_level
        )

        reasoning.append(
            f"Final: score={relevance_score:.2f}, confidence={confidence_level:.2f}, routing={routing_decision}"
        )
    ...
    def _analyze_keyword_matching(
        ...
        try:
            if isinstance(topic.keywords, list):
                ...
                if topic.keywords.startswith("[") and topic.keywords.endswith("]"):
                    ...
                    keywords = [
                        k.strip().lower()
                        for k in json.loads(topic.keywords)
                        if k.strip()
                    ]
                else:
                    ...
                    keywords = [
                        k.strip().lower()
                        for k in topic.keywords.split(",")
                        if k.strip()
                    ]
            else:
                self.logger.warning(
                    f"Unexpected keywords type for topic {topic.name}: {type(topic.keywords)}"
                )
                return 0.0, []
        ...
        text_content = (
            f"{article.title} {article.summary or ''} {article.content or ''}".lower()
        )
        ...
        if self.settings.smart_processing.generic_patterns_enabled:
            ...
            for (
                category,
                patterns,
            ) in self.settings.smart_processing.generic_patterns.items():
                generic_patterns.update(patterns)

            self.logger.debug(
                f"Loaded {len(generic_patterns)} generic patterns from {len(self.settings.smart_processing.generic_patterns)} categories"
            )
        else:
        ...
        self.logger.debug(
            f"Keyword classification - Generic patterns: {len(generic_patterns)}, Topic-specific: {topic_specific_keywords}"
        )
        ...
        for keyword in keywords:
            ...
            specificity_multiplier = (
                0.3 if is_generic else 1.0
            )  # Generic keywords get 30% weight
            ...
            if " " in keyword:
                ...
                if keyword in text_content:
                    ...
                    phrase_score = (
                        self.settings.filtering.exact_phrase_weight * 1.5
                    ...
                    if word_matches > 0:
                        partial_score = (
                            word_matches / len(words)
                        ) * self.settings.filtering.partial_word_weight
                ...
                if word_count > 0:
                    ...
                    tf_score = min(
                        word_count * 0.1, self.settings.filtering.single_word_tf_cap
                    )
        ...
        self.logger.info(
            f"🔧 SEMANTIC ANALYZER v2.0 ACTIVE - Processing: {article.title[:30]}..."
        )

        # 1. If no topic-specific keywords matched, severely penalize the score
        if not matched_topic_specific and matched_keywords:
            self.logger.info(
                f"❌ No topic-specific keywords matched, only generic: {matched_keywords}"
            )
        ...
        if (
            ...
            and matched_keywords[0].lower() in ambiguous_keywords
        ):
            self.logger.debug(
                f"Single ambiguous keyword '{matched_keywords[0]}' - reducing confidence"
            )
        ...
        if "tiktok" in topic_name_lower:
            ...
            has_container_context = any(
                kw.lower() in container_specific for kw in matched_keywords
            )
            if not has_container_context and matched_keywords:
                self.logger.debug(
                    f"ECS/EKS topic without container context, reducing score"
                )
                total_score *= 0.3

        elif (
            "engineering culture" in topic_name_lower
            or "management" in topic_name_lower
        ):
            ...
            has_management_context = any(
                kw.lower() in management_specific for kw in matched_keywords
            )
            if not has_management_context and matched_keywords:
                self.logger.debug(
                    f"Management topic without leadership context, reducing score"
                )
                total_score *= 0.3
    ...
    def _calculate_confidence_level(
        ...
        keyword_weight = 0.6
        other_weight = (
            0.4 / (len(confidence_factors) - 1) if len(confidence_factors) > 1 else 0.4
        )
    ...
    def _make_routing_decision(
        self, relevance_score: float, confidence_level: float
    ) -> str:
        ...
        # These are articles that matched keywords but failed semantic coherence checks
            ...
            self.logger.debug(
                f"Blocking semantically penalized article: score={relevance_score:.3f}, confidence={confidence_level:.3f}"
            )

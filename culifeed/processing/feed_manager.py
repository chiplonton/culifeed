...
class FeedManager:
    ...
    async def add_feed(
        self, chat_id: str, feed_url: str, title: str = None, description: str = None
    ) -> Feed:
        ...
        try:
            ...
            if isinstance(e, FeedManagementError):
                raise
            raise FeedManagementError(
                f"Failed to add feed: {e}", error_code=ErrorCode.DATABASE_ERROR
            )
    ...
    async def _test_feed_access(self, feed_url: str) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(
                feed_url=feed_url, success=False, error=f"Test access failed: {e}"
            )
        ...
        with self.db.get_connection() as conn:
            if fetch_result.success:
                ...
                feed_row = conn.execute(
                    "SELECT error_count FROM feeds WHERE id = ?", (feed_id,)
                ).fetchone()
                if (
                    feed_row
                    and feed_row["error_count"] >= self.settings.limits.max_feed_errors
                ):
    ...
    def _disable_feed(self, conn, feed_id: int, reason: str) -> None:
        ...
        """
        conn.execute("UPDATE feeds SET active = ? WHERE id = ?", (False, feed_id))
    ...
    def get_all_feed_health(self, chat_id: str = None) -> List[FeedHealthStatus]:
        ...
        with self.db.get_connection() as conn:
            if chat_id:
                rows = conn.execute(
                    "SELECT * FROM feeds WHERE chat_id = ?", (chat_id,)
                ).fetchall()
            else:
    ...
    def cleanup_old_articles(self, days_to_keep: int = None) -> int:
        ...
        with self.db.get_connection() as conn:
            ...
            cursor = conn.execute(
                "DELETE FROM processing_results WHERE processed_at < ?", (cutoff_date,)
            )
            results_deleted = cursor.rowcount

            # Then delete old articles
            cursor = conn.execute(
                "DELETE FROM articles WHERE created_at < ?", (cutoff_date,)
            )

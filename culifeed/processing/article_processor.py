...
class ArticleProcessor:
    ...
    def normalize_content(self, article: Article) -> Article:
        ...
        if normalized_article.title:
            ...
            # Clean whitespace
            normalized_article.title = re.sub(
                r"\s+", " ", normalized_article.title
            ).strip()
        ...
        if normalized_article.content:
            ...
            # Remove HTML tags
            normalized_article.content = re.sub(
                r"<[^>]+>", " ", normalized_article.content
            )
            # Clean whitespace and normalize line breaks
            normalized_article.content = re.sub(
                r"\s+", " ", normalized_article.content
            ).strip()
            # Truncate to max length
            if len(normalized_article.content) > self.max_content_length:
                normalized_article.content = (
                    normalized_article.content[: self.max_content_length] + "..."
                )
        ...
        content_for_hash = (
            f"{normalized_article.title}|{normalized_article.url}".encode("utf-8")
        )
    ...
    def find_duplicates_in_database(
        self, articles: List[Article], days_lookback: int = 7
    ) -> List[ProcessingResult]:
        ...
        with self.db.get_connection() as conn:
            # Get existing article hashes and URLs from recently created OR recently published articles
        ...
        for article in articles:
            ...
            # Check if article is too old (older than lookback period)
            if article.published_at and article.published_at < cutoff_date:
                is_duplicate = True
                duplicate_of = f"old_article:{article.published_at.strftime('%Y-%m-%d')}"
                content_issues.append(f"Article too old (published {article.published_at.strftime('%Y-%m-%d')})")
    ...
    def process_articles(
        self, articles: List[Article], check_database: bool = True
    ) -> Tuple[List[Article], DeduplicationStats]:

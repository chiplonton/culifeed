...
class PreFilterEngine:
    ...
    def __init__(self, settings: Optional["CuliFeedSettings"] = None):
        ...
        self.logger.info(
            "Pre-filter engine initialized with configurable thresholds",
            extra={
    ...
    def _calculate_keyword_relevance(
        self, text_features: Dict[str, any], keywords: List[str]
    ) -> float:
        ...
        for keyword in keywords:
            ...
            if keyword in clean_text:
                ...
                phrase_score = min(
                    phrase_count * self.thresholds["exact_phrase_weight"], 1.0
                )  # Cap at 1.0
            ...
            if len(keyword_words) > 1:
                ...
                if word_matches == len(keyword_words):
                    ...
                    word_score = self.thresholds[
                        "partial_word_weight"
                    ]  # Full partial weight when all words found
                ...
                if word in tf_scores:
                    word_score = min(
                        tf_scores[word] * 10, self.thresholds["single_word_tf_cap"]
                    )  # Scale TF and cap
                    keyword_score = max(keyword_score, word_score)
                    matched_keywords += 1
        ...
        match_bonus = (
            min(matched_keywords / len(keywords), 1.0)
            * self.thresholds["keyword_match_bonus"]
        )

        final_score = min(base_score + match_bonus, 1.0)
        return final_score

    def _check_exclusion_keywords(
        self, text_features: Dict[str, any], exclude_keywords: List[str]
    ) -> bool:
    ...
    def filter_articles(
        self, articles: List[Article], topics: List[Topic]
    ) -> List[FilterResult]:
        ...
        self.logger.info(
            f"Pre-filtering {len(articles)} articles against {len(topics)} topics"
        )
        ...
        reduction_percent = (
            ((len(articles) - passed_count) / len(articles)) * 100 if articles else 0
        )
        ...
        for result in results:
            if result.passed_filter:
                filtered_articles.append(
                    (result.article, result.best_match_topic, result.best_match_score)
                )

...
class FeedFetcher:
    ...
    async def fetch_feed(self, feed_url: str, session: aiohttp.ClientSession) -> FetchResult:
        ...
        try:
            ...
            async with session.get(validated_url) as response:
                if response.status != 200:
                    ...
                    self.logger.warning(f"Feed fetch failed for {validated_url}: {error_msg}")
                    return FetchResult(
                        feed_url=feed_url,
                        success=False,
                        error=error_msg,
                ...
                if hasattr(feed_data, 'bozo') and feed_data.bozo:
                    ...
                    if not hasattr(feed_data, 'entries') or not feed_data.entries:
                        self.logger.warning(f"Feed parse failed for {validated_url}: {error_msg}")
                        return FetchResult(
                            feed_url=feed_url,
                            success=False,
                            error=error_msg,
                        ...
                        self.logger.info(f"Feed has parse warnings but contains entries: {validated_url}")
            ...
            self.logger.error(f"Feed fetch failed for {feed_url}: {error_msg}", exc_info=True)
            return FetchResult(
                feed_url=feed_url,
                success=False,
                error=error_msg,
    ...
    def _parse_entries(self, feed_data: Any, feed_url: str) -> List[Article]:
        ...
        for entry in feed_data.entries:
            try:
                ...
                if not article_url:
                    self.logger.warning(f"Entry missing URL in feed {feed_url}, skipping")
                    continue
                ...
                try:
                    ...
                    self.logger.warning(f"Invalid article URL '{article_url}' in feed {feed_url}: {e}")
                    continue
        ...
        for field in content_fields:
            if hasattr(entry, field):
                ...
                if raw_content and isinstance(raw_content, str):
                    try:
                        cleaned_content = ContentValidator.validate_article_content(raw_content)
                        if cleaned_content:
    ...
    async def fetch_feeds(self, feed_urls: List[str]) -> AsyncGenerator[FetchResult, None]:

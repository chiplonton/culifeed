...
class ArticleRepository:
    ...
    def create_article(self, article: Article) -> str:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to create article: {e}", error_code=ErrorCode.DATABASE_ERROR
            ) from e
    ...
    def update_article(self, article_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                cursor = conn.execute(
                    f"UPDATE articles SET {', '.join(set_clauses)} WHERE id = ?", values
                )
                conn.commit()
    ...
    def delete_old_articles(self, days_to_keep: int = None) -> int:
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                ...
                self.logger.info(
                    f"Deleted {deleted_count} articles older than {days_to_keep} days"
                )

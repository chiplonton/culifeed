...
class TopicRepository:
            ...
            with self.db.get_connection() as conn:
                cursor = conn.execute(
                    ...
                    (
                        ...
                        json.dumps(topic.exclude_keywords), topic.confidence_threshold,
                        topic.created_at, topic.last_match_at, topic.active
                    )
                )
                conn.commit()
    ...
    def get_topics_for_chat(self, chat_id: str, active_only: bool = True) -> List[Topic]:
    ...
    def get_topics_for_channel(self, chat_id: str, active_only: bool = True) -> List[Topic]:
            ...
            for field, value in updates.items():
                if field == 'name':
                    ...
                    if isinstance(value, list):
                        ...
                        validated_keywords = [kw.strip().lower() for kw in value if kw.strip()]
    ...
    def update_last_match(self, topic_id: int) -> bool:
        ...
        Returns:
            True if update successful, False otherwise
        """
        return self.update_topic(topic_id, {
    ...
    def delete_topics_for_chat(self, chat_id: str) -> int:
        ...
        try:
            with self.db.get_connection() as conn:
                cursor = conn.execute(
                    "DELETE FROM topics WHERE chat_id = ?",
                    (chat_id,)
                )
                conn.commit()
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                ...
                return {
                    ...
                    'inactive_topics': basic_stats['total_topics'] - basic_stats['active_topics'],
                    'avg_confidence_threshold': round(basic_stats['avg_confidence_threshold'] or 0, 2),
                    'avg_keywords_per_topic': round(keyword_stats['avg_keywords_per_topic'] or 0, 1),
                    'max_keywords_per_topic': keyword_stats['max_keywords_per_topic'] or 0,
                    'topics_with_recent_matches': recent_matches['topics_with_recent_matches']
                }

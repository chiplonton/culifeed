...
class MessageSender:
    ...
    async def deliver_daily_digest(
        self, chat_id: str, limit_per_topic: int = 5
    ) -> DeliveryResult:
        ...
        try:
            ...
            articles_by_topic = await self._get_articles_for_delivery(
                chat_id, limit_per_topic
            )
            ...
            messages_sent = 0
            total_articles = sum(
                len(articles) for articles in articles_by_topic.values()
            )
            ...
            for message in formatted_messages:
                ...
                await asyncio.sleep(
                    self.settings.delivery_quality.message_delay_seconds
                )
    ...
    async def _get_articles_for_delivery(
        self, chat_id: str, limit_per_topic: int
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                if not article_rows:
                    ...
                    self.logger.warning(
                        f"No processing results found for {chat_id}, falling back to all recent articles"
                    )
                ...
                for row in article_rows:
                    ...
                    article_data = {
                        k: v for k, v in row_dict.items() if k != "topic_name"
                    }
                    ...
                    topic_article_count[topic_name] = (
                        topic_article_count.get(topic_name, 0) + 1
                    )

                self.logger.info(
                    f"Retrieved {sum(len(articles) for articles in articles_by_topic.values())} articles across {len(articles_by_topic)} topics for delivery"
                )
    ...
    async def _get_articles_fallback(
        ...
        """
        self.logger.warning(
            f"Fallback triggered for {chat_id} - processing_results table is empty"
        )
        self.logger.warning(
            "Returning empty results to prevent incorrect article delivery"
        )
    ...
    def _format_transparent_digest(
    ...
    async def _send_message(self, chat_id: str, message: str, retries: int = 3) -> bool:
        ...
        for attempt in range(retries):
            try:
                ...
                # Timeout, retry
                self.logger.warning(
                    f"Timeout sending to {chat_id} (attempt {attempt + 1}): {e}"
                )
    ...
    async def _mark_articles_delivered(
        self, chat_id: str, articles_by_topic: Dict[str, List[Article]]
    ) -> None:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for topic_name, articles in articles_by_topic.items():
                    for article in articles:
                        ...
                        if cursor.rowcount > 0:
                            delivered_count += 1
                            self.logger.debug(
                                f"Marked article {article.id} as delivered for topic '{topic_name}'"
                            )
                        else:
                            self.logger.warning(
                                f"Could not mark article {article.id} as delivered - no processing result found"
                            )
                ...
                self.logger.info(
                    f"Marked {delivered_count} articles as delivered for channel {chat_id}"
                )

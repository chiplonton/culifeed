...
class FeedManager:
    ...
    def fetch_feed(self, feed_url: str) -> Tuple[FeedMetadata, List[ParsedArticle]]:
        ...
        try:
            ...
            if self._should_skip_feed(feed_url):
                raise FeedFetchError(f"Feed temporarily disabled due to errors: {feed_url}")
            ...
            self.logger.debug(f"Feed fetched in {fetch_time:.2f}s, size: {len(response.content)} bytes")
            
            # Parse feed content
            parsed_feed = feedparser.parse(response.content, response_headers=dict(response.headers))
            
            # Check for parsing errors (bozo detection)
            if parsed_feed.bozo:
                self.logger.warning(f"Feed parsing warning for {feed_url}: {parsed_feed.bozo_exception}")
            ...
            self.logger.info(f"Successfully parsed {len(articles)} articles from {feed_url}")
    ...
    async def fetch_feed_async(self, feed_url: str, session: aiohttp.ClientSession) -> Tuple[FeedMetadata, List[ParsedArticle]]:
        """
        ...
        try:
            ...
            async with session.get(feed_url, timeout=timeout) as response:
                ...
                fetch_time = time.time() - start_time
                self.logger.debug(f"Feed fetched (async) in {fetch_time:.2f}s, size: {len(content)} bytes")
                
                # Parse feed content
                parsed_feed = feedparser.parse(content, response_headers=dict(response.headers))
                ...
                self._record_successful_fetch(feed_url)
                self.logger.info(f"Successfully parsed {len(articles)} articles from {feed_url} (async)")
    ...
    async def fetch_multiple_feeds(self, feed_urls: List[str]) -> Dict[str, Tuple[FeedMetadata, List[ParsedArticle]]]:
        """
    ...
    def _extract_feed_metadata(self, feed_data: Any, feed_url: str) -> FeedMetadata:
        ...
        try:
            ...
            description = getattr(feed_data, 'description', '') or getattr(feed_data, 'subtitle', '')
            ...
            if hasattr(feed_data, 'updated_parsed') and feed_data.updated_parsed:
                try:
                    updated = datetime(*feed_data.updated_parsed[:6], tzinfo=timezone.utc)
            ...
            if hasattr(feed_data, 'categories'):
                categories = [cat.get('term', '') if isinstance(cat, dict) else str(cat) 
    ...
    def _extract_article(self, entry_data: Any, feed_url: str) -> ParsedArticle:
        ...
        elif hasattr(entry_data, 'author_detail') and isinstance(entry_data.author_detail, dict):
        ...
        if hasattr(entry_data, 'published_parsed') and entry_data.published_parsed:
            try:
                published = datetime(*entry_data.published_parsed[:6], tzinfo=timezone.utc)
            except (ValueError, TypeError):
                pass
        ...
        if hasattr(entry_data, 'tags') and entry_data.tags:
            try:
                ...
                tags_iter = entry_data.tags if hasattr(entry_data.tags, '__iter__') else []
                categories = [tag.get('term', '') if isinstance(tag, dict) else str(tag) 
        ...
        if hasattr(entry_data, 'enclosures') and entry_data.enclosures:
            try:
                ...
                enclosures_iter = entry_data.enclosures if hasattr(entry_data.enclosures, '__iter__') else []
        ...
        if summary and not validate_content_length(summary, self.settings.processing.max_content_length):
            self.logger.warning(f"Article summary too long, truncating: {len(summary)} chars")
            summary = summary[:self.settings.processing.max_content_length] + "..."
        
        if content and not validate_content_length(content, self.settings.processing.max_content_length * 2):
            self.logger.warning(f"Article content too long, truncating: {len(content)} chars")
    ...
    def _record_feed_error(self, feed_url: str) -> None:
        ...
        if current_errors + 1 >= self.settings.limits.max_feed_errors:
            self.logger.error(f"Feed {feed_url} reached maximum error limit ({self.settings.limits.max_feed_errors})")
    ...
    def get_feed_health_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for feed_url in set(list(self._feed_errors.keys()) + list(self._last_fetch_times.keys())):
            status[feed_url] = {
                'error_count': self._feed_errors.get(feed_url, 0),
...
async def fetch_feeds_batch(feed_urls: List[str]) -> Dict[str, Tuple[FeedMetadata, List[ParsedArticle]]]:

...
def get_processing_logger(chat_id: Optional[str] = None, topic_name: Optional[str] = None) -> LoggerAdapter:
    """Get logger for processing components."""
    return get_logger_for_component('processing', chat_id=chat_id, topic_name=topic_name)


def get_ai_logger(article_id: Optional[str] = None, topic_name: Optional[str] = None) -> LoggerAdapter:
...
class PerformanceLogger:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if self.start_time:
            ...
            if exc_type:
                self.logger.error(f"Failed {self.operation} in {duration:.3f}s", extra=context)
            else:

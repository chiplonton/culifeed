...
class AIProvider(ABC):
    ...
    def _create_success_result(self, relevance_score: float, confidence: float,
    ...
    def _build_relevance_prompt(self, article: Article, topic: Topic) -> str:
        ...
        """
        keywords_str = ", ".join(topic.keywords) if topic.keywords else "general interest"
        exclude_str = f"\nEXCLUDE articles about: {', '.join(topic.exclude_keywords)}" if topic.exclude_keywords else ""
...
SUMMARY:"""
    ...
    def _parse_relevance_response(self, response_text: str) -> tuple[float, float, List[str], str]:
        ...
        for line in lines:
            ...
            if line.startswith('RELEVANCE_SCORE:'):
                ...
                if keywords_str and keywords_str != '[]':
                    ...
                    matched_keywords = [k.strip() for k in keywords_str.split(',') if k.strip()]
